...
import Resources.Recursos.TroncoTemplate;
...
{ 
    ...
    private Actor player;                                                   //posicion del player (para las transparencias)
    ...
    public PixieArbol (TroncoTemplate troncoTemplate, Actor player) { tronco = new Tronco( troncoTemplate ); this.player = player; }
    public PixieArbol (Tronco troncoTemplate, Actor player)         { tronco = new Tronco( troncoTemplate ); this.player = player; }
    public PixieArbol (int numTronco, Actor player)                 { tronco = new Tronco( Recursos.listaDeTroncos.get(numTronco)); this.player = player; }
    
    //CONSTRUCTOR COPIA: construye un arbol copiandolo a partir del que se le pasa como parametro
    public PixieArbol (PixieArbol arbol, Actor player)
    {
    ...
    public void draw (Batch batch, float alpha)
    {
        ...
        double distancia = Math.abs(Math.sqrt(Math.pow(player.getX()-getX(),2)+Math.pow(player.getY()-getY()-80, 2)));
