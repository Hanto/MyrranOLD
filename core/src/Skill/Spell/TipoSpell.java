...
public abstract class TipoSpell implements TipoSpellInterface
{
    ...
    {   //Establecemos el punto de salida del pepo, suele ser el centro de gravedadad del Pixie (altura/2) (anchura/2):
        ...
        {   //Personaje Jugador:
            ...
            return new Vector2(caster.getActor().getX()+caster.getActor().getWidth()/2, caster.getActor().getY()+caster.getActor().getHeight()/2);
        } 
        else //NPC  
        {
            return new Vector2(caster.getActor().getX(), caster.getActor().getY());
        }
    }
